...
```math
\{a_g\}^* = \text{argmax}_{\vec \{a_g\} \in \{0,1\}^{|G|}} U(a, I)
```
...
The relevant number here is the way in which contacts are performed. Individuals will, on average, contact $c$ other agents, yet, the number of agents to contact will change throughout the simulation as a function of how each community's contact rate are specified. Particularly, the number of *infected* contacts an agent from group $g$ will make from group $l$, $k(g,l)$, has a binomial distribution with the following parameters:
